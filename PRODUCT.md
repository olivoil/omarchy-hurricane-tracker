...
Omarchy users who live in, travel through, or care about hurricane-prone regions. They may glance at the bar during an ordinary workday or open the full tracker while weather is developing. Their primary job is to understand which tropical cyclones are active, which disturbances may develop in their region, where systems are moving, how fresh the official guidance is, and whether a forecast has become relevant to a saved home, family, or travel location.

## Product Purpose

Hurricane Tracker is a native Omarchy tropical-system tracker. A compact bar status opens a spatial, source-forward view of National Hurricane Center cyclones, forecast tracks, uncertainty cones, developing areas, discussions, and locally saved watch places. Success means a user can scan a region, understand a selected system in seconds, and opt into calm threshold-based basin or place awareness without mistaking the plugin for local emergency guidance.
...
4. Organize by place: preserve NHC's basin hierarchy while letting users watch named personal locations without implying unsupported global coverage.
5. Reveal detail progressively: start with regional systems, then expose the selected track, cone or formation area, discussion, and official links.
